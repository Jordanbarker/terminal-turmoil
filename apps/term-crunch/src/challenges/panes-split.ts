...
function buildTargetWindow(): WindowState {
  ...
  const first = splitNode(win.root, win.activePaneId, "h", () => makeLeaf(CRUNCH_MACHINE, HOME_DIR));
  if (!first) throw new Error("panes-split: horizontal split failed");
  const second = splitNode(first.root, first.newPaneId, "v", () => makeLeaf(CRUNCH_MACHINE, HOME_DIR));
