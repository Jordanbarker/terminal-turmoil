...
- **`CommandContext.devices`** (`devices.ts`) — block devices; `df` reads size + Filesystem column from `rootDevice()`, so df and lsblk can't disagree.
...
All have a `reset*` counterpart for test isolation. Importing termoil's `builtins/index.ts` gives tests/the app the complete command layer, seams included.
