...
- **`tmux` builtin** `@tt/core/commands/builtins/tmux.ts` — pure handler validating against `CommandContext.tmux` (app-injected snapshot; detached sessions **most-recent-last**) and returning a fully resolved `CommandResult.tmuxAction` (`new-session`/`attach`/`detach`/`rename-session`/`kill-session`/`kill-server`). Real-tmux errors (nested guard, `no server running`, `can't find session`, `duplicate session`). `computeEffects` passes `tmuxAction` through; each app's `applyTmuxAction(action): boolean` store action applies it (true = client view swapped → suppress the prompt; the fresh pane prints the one-shot `pendingMuxNotice` banner — `[detached (from session X)]`/`[exited]`/`[server exited]` — via `onPaneCreated`).
...
- **Pane-runtime lifecycle** — two effects own it, and they must stay symmetric. The `[windows]` effect creates/disposes runtimes as panes enter/leave the tree; a separate `[]`-scoped effect disposes **all** of them on unmount (the imperative containers and their `mousedown` listeners aren't React-owned, so nothing else would). `disposeRuntime` must undo everything `createPaneRuntime` set up — listener, `onData` disposable, xterm, container node. The unmount teardown also **resets the first-mount bookkeeping** (`knownPaneIdsRef`, `firstPaneConsumedRef`): Next dev runs StrictMode's mount → cleanup → mount, and if that state survived, pass 2 would recreate every pane as `kind:"restored"`/`firstPane:false` and the app would silently skip its splash/intro. Any new "have we booted yet" ref belongs in that reset.
