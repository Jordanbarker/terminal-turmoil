...
- **Edward Torres**: Manager at NexaCorp. Well-meaning but non-technical. His impression of the player (`edward_impression` flag) is set by how they respond to the job offer.
- **Jin Chen**: Previous senior engineer who left abruptly. Clues in `/home/jchen/` on NexaCorp. Full name in formal data (database records, HR documents, dbt output).
- **Chip**: NexaCorp's AI assistant (Collaborative Helper for Internal Processes).
