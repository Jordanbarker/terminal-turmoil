...
| 2025-06-01 | **Initial Chip rollout begins** — first plugin batch lands alongside `chip_service_account` provisioning. See Section E for current-version install dates. | `chipinfra/opt.ts:57-66`, `chipinfra/srv.ts:261` |
...
| 2026-02-24 ~21:14 | Marcus board-meeting debrief — auto-opens in `less` when player runs `exit` from NexaCorp post-accusation; closes Chapter 3 (`read_board_debrief_day2`) | `src/story/marcusDebrief.ts`, `src/engine/commands/builtins/exit.ts` |
...
### E. Chipinfra plugin install dates (`src/story/filesystem/chipinfra/opt.ts:57-66`)

`installed` reflects the **current version's** install timestamp (matches `apt`/`dpkg` semantics), not the first-ever install of the plugin. The original Chip rollout began 2025-06-01 (see Tier 1 service-account row); every plugin below has been on the platform since at most ~2 months after that, but the registry surfaces only the latest version bump.

| Date | Plugin | Version | Notes |
|---|---|---|---|
| 2025-11-15 | alert-routing | 1.3.1 | 3 minors + 1 patch |
| 2025-11-22 | code-review | 1.4.0 | 4 minor bumps |
| 2025-12-10 | onboarding | 1.2.0 | 2 minor bumps |
| 2025-12-18 | incident-response | 1.0.3 | 3 patches |
| 2026-01-22 | analytics-reports | 2.3.0 | Major-version bump + 3 minors |
| **2026-01-28** | **log-maintenance** | **1.1.0** | **v1.1.0 introduces the "cleanup" pass that strips `chip_service_account` from `system.log` (preserved in `system.log.bak`). Installed ~1 week before Jin's resignation / exfil window — suspicious in retrospect.** |
| 2026-01-30 | system-monitor | 2.0.0 | Clean major-version release |
| 2026-02-05 | ticket-triage | 1.5.2 | 5 minors + 2 patches — actively maintained |
| 2026-02-09 | data-pipeline | 3.0.1 | Two majors + a patch |
| 2026-02-12 | brand-voice | 2.1.0 | Major + minor |
...
| 2026-02-02 (file says interview is "tomorrow!" — Feb 3) | interview prep file last edited | `downloads.ts:146` |
