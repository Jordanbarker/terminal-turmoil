...
const umount: CommandHandler = (args, _flags, ctx) => {
  if (args.length !== 1) {
    return errorResult("umount: bad usage\nTry 'umount --help' for more information.", 1);
  }

  const found = findMount(args[0], ctx);
  if (!found) {
    return errorResult(`umount: ${args[0]}: not mounted`, 1);
  }
  ...
  if (insertResult.error || !insertResult.fs) {
    return errorResult(`umount: ${insertResult.error ?? "failed"}`, 1);
  }
