...
import { AsyncCommandHandler } from "@tt/core/commands/types";
...
const pythonHandler: AsyncCommandHandler = async (args, flags, ctx) => {
  ...
  if (args.length > 0) {
    ...
    // The app may want this script to produce authored output rather than
    // actually be interpreted (see scriptInterceptors.ts).
