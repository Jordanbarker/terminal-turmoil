...
const tail: CommandHandler = (args, _flags, ctx) => {
  ...
  for (let i = 0; i < effectiveArgs.length; i++) {
    if (effectiveArgs[i] === "-f" || effectiveArgs[i] === "--follow") {
      return errorResult("tail: -f: follow not supported in this terminal", 2);
    }
    if (effectiveArgs[i] === "-n") {
      if (i + 1 >= effectiveArgs.length) {
        return errorResult("tail: option requires an argument -- 'n'", 2);
      }
      numLines = parseInt(effectiveArgs[i + 1], 10);
      if (isNaN(numLines) || numLines < 0) {
        return errorResult(`tail: invalid number of lines: '${effectiveArgs[i + 1]}'`, 2);
      }
      i++;
  ...
  if (fileArgs.length === 0) {
    return errorResult("tail: missing file operand", 2);
  }
  ...
  for (const fileArg of fileArgs) {
    ...
    if (isBinaryFile(node)) {
      ...
      errors.push(`tail: ${fileArg}: binary file${hint}`);
      continue;
    }
